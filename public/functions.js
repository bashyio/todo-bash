...
const tryFetch = (url, options, i) =>
  fetch(url, options)
    .then((response) => {
      if (response.status < 200 || response.status > 299)
        throw Error("Request Failed! Status Code: " + response.status);
      return response;
    })
    .catch((error) => {
      if (i <= 1) throw error;
...
const fetchList = async (init = 0) => {
  let list,
    listLocal = parseJSON(localStorage.getItem("toDoList"));
...
const fetchRequest = () => {
  const result = tryFetch(baseUrl + "data-io?rType=fetchList", {}, 3)
    .then((response) => {
...
const saveRequest = (toDoList) => {
  const result = tryFetch(
    baseUrl + "data-io?rType=saveList",
    {
      method: "POST",
      headers: {
...
const renderList = (toDoList) => {
  ...
  if (toDoList && toDoList.toDos.length) {
    ...
    emptyEl(empty);
    empty.appendChild(
      document.createTextNode("List items will display here...")
    );
  }
};
